...
export default function ContactWhatsApp() {
  return (
    <section aria-labelledby="whatsapp-heading" className="rounded-2xl border border-[#25D366]/25 bg-[#25D366]/5 p-6 sm:p-7">
      ...
      {/* Bouton réduit au logo : un halo vert pulse doucement autour, le logo
          s'anime au survol. Le libellé reste pour les lecteurs d'écran. */}
      <a
        href={WHATSAPP_URL}
        target="_blank"
        rel="noopener noreferrer"
        aria-label={WHATSAPP_LABEL}
        className="group relative mx-auto mt-6 flex h-20 w-20 items-center justify-center rounded-full bg-[#25D366] text-charcoal shadow-[0_12px_30px_-10px_rgba(37,211,102,0.7)] transition-transform duration-300 ease-out hover:scale-110 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#25D366] motion-safe:hover:-rotate-6"
      >
        <span aria-hidden="true" className="absolute inset-0 rounded-full bg-[#25D366]/60 motion-safe:animate-halo" />
        <span aria-hidden="true" className="absolute inset-0 rounded-full bg-[#25D366]/40 motion-safe:animate-halo [animation-delay:0.9s]" />
        <svg aria-hidden="true" viewBox="0 0 32 32" className="relative h-10 w-10 fill-current transition-transform duration-300 ease-out group-hover:scale-110">
